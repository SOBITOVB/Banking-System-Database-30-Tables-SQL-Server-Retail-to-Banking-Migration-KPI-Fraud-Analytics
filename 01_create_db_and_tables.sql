...
DROP TABLE IF EXISTS bank.AMLCases;
...
CREATE TABLE bank.AMLCases
(
    AMLCaseId              BIGINT IDENTITY(1,1) NOT NULL,
    CustomerId             INT NOT NULL,
    TransactionId          BIGINT NULL,
    FraudCaseId            BIGINT NULL,
    CaseOpenedAt           DATETIME2(0) NOT NULL,
    CaseClosedAt           DATETIME2(0) NULL,
    AMLStatus              VARCHAR(20) NOT NULL,
    RiskRating             VARCHAR(10) NOT NULL,
    AlertSource            VARCHAR(30) NOT NULL,
    Notes                  NVARCHAR(300) NULL,
    CreatedByEmployeeId    INT NULL,
    CreatedAt              DATETIME2(0) NOT NULL CONSTRAINT DF_AMLCases_CreatedAt DEFAULT SYSUTCDATETIME(),
    UpdatedAt              DATETIME2(0) NOT NULL CONSTRAINT DF_AMLCases_UpdatedAt DEFAULT SYSUTCDATETIME(),
    CONSTRAINT PK_AMLCases PRIMARY KEY CLUSTERED (AMLCaseId),
    CONSTRAINT CK_AMLCases_Dates CHECK (CaseClosedAt IS NULL OR CaseClosedAt >= CaseOpenedAt),
    CONSTRAINT CK_AMLCases_Status CHECK (AMLStatus IN ('Open', 'Investigating', 'Escalated', 'Closed', 'FalsePositive')),
    CONSTRAINT CK_AMLCases_Risk CHECK (RiskRating IN ('Low', 'Medium', 'High', 'Critical')),
    CONSTRAINT CK_AMLCases_Source CHECK (AlertSource IN ('TransactionMonitoring', 'FraudRule', 'ManualReview')),
    CONSTRAINT CK_AMLCases_Reference CHECK (TransactionId IS NOT NULL OR FraudCaseId IS NOT NULL),
    CONSTRAINT FK_AMLCases_Customers FOREIGN KEY (CustomerId) REFERENCES bank.Customers (CustomerId),
    CONSTRAINT FK_AMLCases_Transactions FOREIGN KEY (TransactionId) REFERENCES bank.[Transactions] (TransactionId),
    CONSTRAINT FK_AMLCases_FraudDetection FOREIGN KEY (FraudCaseId) REFERENCES bank.FraudDetection (FraudCaseId),
    CONSTRAINT FK_AMLCases_Employees FOREIGN KEY (CreatedByEmployeeId) REFERENCES bank.Employees (EmployeeId)
);
GO
...
CREATE INDEX IX_AMLCases_CustomerId ON bank.AMLCases (CustomerId);
CREATE INDEX IX_AMLCases_TransactionId ON bank.AMLCases (TransactionId);
CREATE INDEX IX_AMLCases_FraudCaseId ON bank.AMLCases (FraudCaseId);
CREATE INDEX IX_AMLCases_Status ON bank.AMLCases (AMLStatus);
CREATE INDEX IX_AMLCases_CaseOpenedAt ON bank.AMLCases (CaseOpenedAt);
