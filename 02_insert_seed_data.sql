...
SELECT
    ...
    CASE
        ...
        ELSE 'UZS'
    END,
    CAST(bal.OpenedDateTime AS DATE),
    NULL,
    bal.CurrentBalance,
...
(
    SELECT
        DATEADD(MINUTE, ABS(CHECKSUM(NEWID())) % 1440, DATEADD(day, -ABS(CHECKSUM(NEWID())) % 730, GETDATE())) AS OpenedDateTime,
        CAST(CAST((ABS(CHECKSUM(NEWID())) % 500000) / 10.0 AS DECIMAL(18,2)) + 100.00 AS DECIMAL(19,2)) AS CurrentBalance,
        CAST((ABS(CHECKSUM(NEWID())) % 20000) / 100.0 AS DECIMAL(19,2)) AS HoldAmount
) bal
WHERE n.n <= 25000;
...
(
    SELECT
        CAST(
            CASE
                WHEN n.n % 200 = 0 THEN 10000 + CAST((ABS(CHECKSUM(NEWID())) % 500000) / 10.0 AS DECIMAL(18,2))
                WHEN n.n % 25 = 0 THEN 500 + CAST((ABS(CHECKSUM(NEWID())) % 500000) / 20.0 AS DECIMAL(18,2))
                ELSE CAST((ABS(CHECKSUM(NEWID())) % 500000) / 10.0 AS DECIMAL(18,2))
            END
        AS DECIMAL(19,2)) AS Amount,
        DATEADD(MINUTE, ABS(CHECKSUM(NEWID())) % 1440, DATEADD(day, -ABS(CHECKSUM(NEWID())) % 730, GETDATE())) AS TransactionDate
) tx
WHERE n.n <= 240000;
...
SELECT
    ...
    ((n.n - 1) % 500) + 1,
    ((n.n - 1) % 6) + 1,
    CAST(CASE WHEN n.n % 150 = 0
        THEN 1500 + CAST((ABS(CHECKSUM(NEWID())) % 500000) / 10.0 AS DECIMAL(18,2))
        ELSE CAST((ABS(CHECKSUM(NEWID())) % 500000) / 10.0 AS DECIMAL(18,2))
    END AS DECIMAL(19,2)),
    CASE WHEN n.n % 10 IN (0, 1, 2, 3, 4, 5) THEN 'USD' WHEN n.n % 10 IN (6, 7) THEN 'EUR' ELSE 'GBP' END,
    DATEADD(MINUTE, ABS(CHECKSUM(NEWID())) % 1440, DATEADD(day, -ABS(CHECKSUM(NEWID())) % 730, GETDATE())),
...
(
    SELECT
        CAST(5000 + CAST((ABS(CHECKSUM(NEWID())) % 500000) / 10.0 AS DECIMAL(18,2)) AS DECIMAL(19,2)) AS PrincipalAmount,
        ...
        CASE n.n % 6
            ...
            ELSE 84
        END AS TermMonths,
        CAST(DATEADD(day, -ABS(CHECKSUM(NEWID())) % 730, GETDATE()) AS DATE) AS StartDate
) l
WHERE n.n <= 12000;
...
SELECT TOP (900)
    ...
    fd.FraudCaseId,
    fd.DetectionTime,
    CASE WHEN s.AMLStatus IN ('Closed', 'FalsePositive') THEN DATEADD(HOUR, 8, fd.DetectionTime) ELSE NULL END,
    s.AMLStatus,
    CASE
        WHEN fd.RiskScore >= 90 THEN 'Critical'
        WHEN fd.RiskScore >= 80 THEN 'High'
        WHEN fd.RiskScore >= 65 THEN 'Medium'
        ELSE 'Low'
    END,
    'FraudRule',
    N'Auto AML case generated from fraud detection.',
    ((fd.FraudCaseId - 1) % 1200) + 1,
    SYSUTCDATETIME(),
    SYSUTCDATETIME()
FROM bank.FraudDetection fd
OUTER APPLY
(
    SELECT TOP (1) ah.CustomerId
    FROM bank.[Transactions] t
    JOIN bank.AccountHolders ah
        ON ah.AccountId = t.AccountId
       AND ah.IsPrimary = 1
       AND ah.EndDate IS NULL
    WHERE t.TransactionId = fd.TransactionId
) txc
OUTER APPLY
(
    ...
    FROM bank.CreditCardTransactions cct
    JOIN bank.Cards c
        ON c.CardId = cct.CardId
    WHERE cct.CardTransactionId = fd.CardTransactionId
) ccc
CROSS APPLY
(
    SELECT
        COALESCE(txc.CustomerId, ccc.CustomerId) AS CustomerId,
        CASE
            WHEN fd.FraudCaseId % 11 = 0 THEN 'FalsePositive'
            WHEN fd.FraudCaseId % 7 = 0 THEN 'Closed'
            WHEN fd.FraudCaseId % 5 = 0 THEN 'Escalated'
            WHEN fd.FraudCaseId % 3 = 0 THEN 'Investigating'
            ELSE 'Open'
        END AS AMLStatus
) s
WHERE s.CustomerId IS NOT NULL
ORDER BY fd.RiskScore DESC, fd.DetectionTime DESC;
...
SELECT TOP (250)
    ...
    t.TransactionId,
    NULL,
    DATEADD(MINUTE, 3, t.TransactionDate),
    CASE WHEN t.TransactionId % 6 = 0 THEN DATEADD(HOUR, 4, t.TransactionDate) ELSE NULL END,
    CASE WHEN t.TransactionId % 6 = 0 THEN 'Closed' WHEN t.TransactionId % 4 = 0 THEN 'Escalated' ELSE 'Open' END,
    CASE WHEN t.Amount > 20000 THEN 'Critical' WHEN t.Amount > 15000 THEN 'High' ELSE 'Medium' END,
    'TransactionMonitoring',
    N'Auto AML case generated from high-value transaction monitoring.',
    ((t.TransactionId - 1) % 1200) + 1,
    SYSUTCDATETIME(),
    SYSUTCDATETIME()
FROM bank.[Transactions] t
